...
ruta_entrada = r'D:\Mis Documentos\Proyectos\ProcesoEditorial\SCALA\Archivos\Produccion\Archivos\Html Demo'   

# Ruta de salida (donde se guardarán los .sql) carpeta destino
ruta_salida = r'D:\Mis Documentos\Proyectos\ProcesoEditorial\SCALA\Archivos\Produccion\SQL\Sql Demo'    
...
for archivo in os.listdir(ruta_entrada):
    if archivo.endswith(".html"):
        ...
        if partes[0].isdigit():
            ...
            id_tarea = partes[0]
            base = partes[1].lower()
            campo = 'MensajeFinalizacionDemo' if 'finalizaciondemo' in base else 'MensajeBienvenidaDemo'

            query = f"""UPDATE TareaEnvioCorreo
            SET {campo} = '{contenido_html}'
            WHERE codfilial = 6 AND Id_TareaEnvioCorreo = {id_tarea};"""
            ...
            query = f"""UPDATE TareaEnvioCorreo
                        SET {campo} = '{contenido_html}'
                        WHERE codfilial = 6 AND PlantillaMail = '{plantilla}';"""
